...
SELECT DNAME AS CARGOS, SUM(SAL) TOTAL_SALÁRIO FROM EMP ORDER BY DEPT;

SELECT * FROM EMP WHERE ENAME = 'SMITH' AND 'TURNER';

 SELECT JOB, AVG(SAL) FROM EMP GROUP BY SAL;
 
 UPDATE EMP SET SAL = 400 WHERE ENAME = 'SMITH';
 
 UPDATE EMP SET DEPTNO = (SELECT DNAME FROM DEPT WHERE DNAME = 'OPERATIONS') 
 WHERE ENAME = 'SMITH';

SELECT DEPTNO, JOB FROM EMP ORDER BY JOB; 

SELECT SAL, MIN(SAL), MAX(SAL), ROUND(AVG(SAL), 2) FROM EMP;

DELETE FROM EMP WHERE HIRADATE = '87';
ROLLBACK;

 UPDATE EMP SET DEPTNO = (SELECT DNAME FROM DEPT WHERE DNAME = 'ACCOUNTING') 
 WHERE ENAME = 'SMITH';
 
 SELECT ENAME FROM EMP WHERE EMPNO AND SAL ORDER BY ENAME; 
